...
public class virementsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boolean faireVersement =false ;
		if (debiter[1].equals("true")) {
			double solde = service.getCompteCourantByID(Integer.parseInt(debiter[0])).getSolde();
			double decouvert = service.getCompteCourantByID(Integer.parseInt(debiter[0])).getDecouvert();
			if (Double.parseDouble(montant) <= solde + decouvert) {
				faireVersement=true;
				service.moneyPayCompteCourant(Integer.parseInt(debiter[0]), Double.parseDouble(montant));
			}
		} else {
			double solde = service.getCompteEpargneByID(Integer.parseInt(debiter[0])).getSolde();
			if (Double.parseDouble(montant) <= solde) {
				faireVersement=true;
		...
		resp.sendRedirect(this.getServletContext().getContextPath() + "/virements?idClient="
				+ req.getParameter("idClient") + "&idCsl=" + req.getParameter("idCsl"));
